...
;(function(factory) {
    ...
    EvoCalendar = (function() {
        ...
        function EvoCalendar(element, settings) {
            ...
            //Anustha's check: console.log("%c Event named: \""+_.options.calendarEvents[1].date+"\" doe");
    ...
    EvoCalendar.prototype.limitTitle = function(title, limit) {
        var newTitle = [];
        limit = limit === undefined ? 64 : limit;
    ...
    // v1.0.0 - Get dates between two dates //Anustha was here
    ...
    EvoCalendar.prototype.isBetweenDates = function(active_date, dates) {
        ...
        if (dates instanceof Array) {
            sd = new Date(_.formatDate(dates[0], _.options.format, _.options.language));
            ed = new Date(_.formatDate(dates[1], _.options.format, _.options.language));
        } else {
            sd = new Date(dates);
            ed = new Date(dates);
        }
...
//Anustha's snooping
            ...
            // console.log("%c Event date: \""+_.options.calendarEvents[0].date+"\" doe")
        }
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
        ...
        var _ = this, markup;
        console.log(_.options.colors.get(event_data.type));
        var eventListEl = _.$elements.eventEl.find('.event-list');
        if (eventListEl.find('[data-event-index]').length === 0) eventListEl.empty();
        _.$active.events.push(event_data);
        markup = '<div class="event-container" onclick="window.open(\'' + event_data.url + '\', \'_blank\');" role="button" data-event-index="'+(event_data.id)+'">';
        markup += '<div class="event-icon"><div class="event-bullet-'+_.options.colors.get(event_data.type)+'"></div></div>';
        markup += '<div class="event-info"><p>'+_.limitTitle(event_data.name)+' </p></div>';
        markup += '<div class="event-info2"><p>'+ _.formatDate(event_data.date[0], _.options.format)+ ' to ' +   _.formatDate(event_data.date[1], _.options.format) + ' </p></div>';
        markup += '</div>';
        eventListEl.append(markup);
    ...
    EvoCalendar.prototype.addEventIndicator = function(event) {
        ...
        function appendDot(date) {
            ...
            //made changes here to map types to color
            if (thisDate.find('span.event-indicator > .type-bullet > .type-'+_.options.colors.get(type)).length === 0) {
                htmlToAppend = '<div class="type-bullet"><div class="type-'+_.options.colors.get(type)+'"></div></div>';
